...
$query="select * from images where md5(id)='$flowchart_id'";
$result=mysqli_query($con,$query);
$row=mysqli_fetch_array($result);
$img_name1=$row['flowchart_image1'];
		$img_name2=$row['flowchart_image2'];
		$pdf_name=$row['flowchart_pdf'];
	...
	$stmt = $con->prepare("Update `images` set `image`=?,`editor_code`=?,`flowchart_name`=?,`flowchart_category`=?,`flowchart_description`=?,`flowchart_image1`=?,`flowchart_image2`=?,`flowchart_iframe`=?,`flowchart_pdf`=? where md5(id)=?");
	$stmt->bind_param('ssssssssss',$img_name,$image_code,$flowchart_name,$flowchart_category,$flowchart_description,$img_name1,$img_name2,$flowchart_iframe,$pdf_name,$flowchart_id);
	$stmt->execute();	
$query="select id from images where md5(id)='$flowchart_id'";
$res=mysqli_query($con,$query);
